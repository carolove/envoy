...
#include "extensions/filters/network/dubbo_proxy/filters/pass_through_filter.h"
...
/**
 * Global configuration for Dubbo rate limit filter.
 */
class Config {
public:
  Config(const envoy::extensions::filters::network::dubbo_proxy::filters::ratelimit::v3::RateLimit&
             config,
...
/**
 * Dubbo rate limit filter instance. Calls the rate limit service with the given configuration
 ...
 */
class Filter : public DubboProxy::DubboFilters::PassThroughDecoderFilter,
...
public:
  ...
  // DubboFilters::PassThroughDecoderFilter
...
private:
  void initiateCall(const DubboProxy::MessageMetadata& metadata);
  void populateRateLimitDescriptors(const DubboProxy::Router::RateLimitPolicy& rate_limit_policy,
                                    std::vector<RateLimit::Descriptor>& descriptors,
                                    const DubboProxy::Router::RouteEntry* route_entry,
                                    const DubboProxy::MessageMetadata& headers) const;
